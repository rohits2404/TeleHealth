...
export async function retrieveAllPatients(queryOptions, fields, queryAtributes) {
  try {
    let personalize = "";
    let query = `SELECT `;
    if (fields) {
...
export async function retrieveAllDoctors(queryOptions, fields, queryAtributes) {
  try {
    let query = `SELECT `;
...
export async function changePersonState(user, id, state) {
  try {
...
export async function retrieveApppointmentsDetails(id, flag) {
  try {
